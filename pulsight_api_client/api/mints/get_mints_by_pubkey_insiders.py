...
def _get_kwargs(
    ...
    *,
    ...
    _kwargs: dict[str, Any] = {
        "method": "get",
        "url": "/api/mints/{pubkey}/insiders".format(
...
def sync_detailed(
    ...
    *,
    ...
    """List Mint Insiders
     ...
     Insiders of one mint (wallets reached from the creator over launch-window token transfers, custodial
    hops excluded) — the SAME set `/risk` counts — in the top-traders row shape. Empty until the mint's
    first fold when token-program capture has no transfers for it. sort ∈
    {balance,holding_pnl,pnl,volume,swaps,recent} (default balance). Paged via offset. Login required
